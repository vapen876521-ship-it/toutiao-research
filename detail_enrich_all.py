...
async def extract(page):
 ...
 return {selector:best?best.s:'body',text:(best?best.t:(document.body.innerText||'')).slice(0,15000),paragraphs:ps.slice(0,120),images:[...new Set(imgs)].slice(0,80),attrs:attrs.slice(0,1500),links:links.slice(0,20)};
 }"""
 ...
 for x in attrs:
  for s in [x.get('aria',''),x.get('title',''),x.get('text','')]:
   ...
   for p in [r'点赞\s*([\d.,]+(?:万|亿)?)',r'([\d.,]+(?:万|亿)?)\s*赞']:
    m=re.search(p,s)
    if m:likes.append(cnnum(m.group(1)))
   for p in [r'评论\s*([\d.,]+(?:万|亿)?)',r'([\d.,]+(?:万|亿)?)\s*(?:条)?评论']:
    m=re.search(p,s)
    if m:comments.append(cnnum(m.group(1)))
 return max(likes or [0]),max(comments or [0])

async def main():
 rows=list(csv.DictReader(IN.open(encoding='utf-8-sig',newline='')))
 mine=[r for i,r in enumerate(rows) if i%SHARDS==SHARD]
 fields=list(rows[0].keys())+['http_status','final_url','detail_verified','page_title','content_selector','content_chars','paragraph_count','content_text','detail_image_count','detail_image_urls','semantic_digg_count','semantic_comment_count','net_digg_count','net_comment_count','net_forward_count','net_repin_count','net_read_count','detail_media_name','detail_user_id','detail_author_token','detail_followers_count','author_links','detail_error']
 output=[]
 async with async_playwright() as p:
  ...
  for j,r in enumerate(mine,1):
   ...
   try:
    ...
    d=await extract(page);sl,sc=semantic(d['attrs']);bn=best_net(net)
    final=page.url;status=resp.status if resp else 0
    rec.update({'http_status':status,'final_url':final,'detail_verified':bool(status==200 and gid in final and len(d['text'])>200),'page_title':await page.title(),'content_selector':d['selector'],'content_chars':len(d['text']),'paragraph_count':len(d['paragraphs']),'content_text':d['text'],'detail_image_count':len(d['images']),'detail_image_urls':json.dumps(d['images'],ensure_ascii=False),'semantic_digg_count':sl,'semantic_comment_count':sc,'net_digg_count':num(bn.get('digg_count')),'net_comment_count':num(bn.get('comment_count')),'net_forward_count':num(bn.get('forward_count')),'net_repin_count':num(bn.get('repin_count')),'net_read_count':num(bn.get('read_count')),'detail_media_name':bn.get('media_name') or '','detail_user_id':bn.get('user_id') or '','detail_author_token':bn.get('author_token') or '','detail_followers_count':num(bn.get('followers_count') or bn.get('follower_count')),'author_links':json.dumps(d['links'],ensure_ascii=False),'detail_error':''})
   except Exception as e:
    rec.update({'http_status':0,'final_url':page.url,'detail_verified':False,'page_title':'','content_selector':'','content_chars':0,'paragraph_count':0,'content_text':'','detail_image_count':0,'detail_image_urls':'[]','semantic_digg_count':0,'semantic_comment_count':0,'net_digg_count':0,'net_comment_count':0,'net_forward_count':0,'net_repin_count':0,'net_read_count':0,'detail_media_name':'','detail_user_id':'','detail_author_token':'','detail_followers_count':0,'author_links':'[]','detail_error':repr(e)[:500]})
 ...
 summary={'shard':SHARD,'rows':len(output),'verified':sum(bool(x.get('detail_verified')) for x in output),'failed':sum(not bool(x.get('detail_verified')) for x in output),'body_ge_1000':sum(num(x.get('content_chars'))>=1000 for x in output),'semantic_like_resolved':sum(num(x.get('semantic_digg_count'))>0 for x in output),'semantic_comment_resolved':sum(num(x.get('semantic_comment_count'))>0 for x in output),'net_metrics_resolved':sum(any(num(x.get(k))>0 for k in ['net_digg_count','net_comment_count','net_forward_count','net_repin_count']) for x in output),'author_link_resolved':sum(x.get('author_links') not in ('','[]') for x in output),'followers_resolved':sum(num(x.get('detail_followers_count'))>0 for x in output)}
