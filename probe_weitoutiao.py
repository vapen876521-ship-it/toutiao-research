...
KEYWORDS=['真实经历','夫妻','婚姻','婆媳','亲子','育儿','中年生活','退休生活','养老','独居','人情世故','职场','打工人','跳槽','面试','辞职','失业','工资','办公室','副业','开店','摆摊','装修','租房','买房','邻居','物业','家务','收纳','厨房','早餐','晚餐','美食','外卖','菜市场','食品安全','省钱','网购','消费体验','旅行','自驾游','酒店','农村生活','县城生活','猫','狗','养猫','养狗','手机','AI','新能源车','新手司机','大学生活','毕业生','家庭教育','生活妙招','暖心故事','网友热议']
...
def parse_obj(d):
    ...
    schema=str(d.get('inner_schema') or detail or '')
    native=('toutiao.com' in url or url.startswith('sslocal://thread_detail') or schema.startswith('sslocal://thread_detail'))
    if not native and not (media and media_url and ('toutiao.com' in media_url or media_url.startswith('/'))):return None
    return {'group_id':str(d.get('group_id')),'title':re.sub(r'\s+',' ',title)[:1200],'abstract':re.sub(r'\s+',' ',str(d.get('abstract') or d.get('description') or ''))[:2500],'article_url':url,'media_name':media,'media_url':media_url,'user_id':str(d.get('user_id') or d.get('media_creator_id') or ''),'publish_time':n(d.get('publish_time') or d.get('create_time') or d.get('behot_time')),'read_count':n(d.get('read_count')),'digg_count':n(d.get('digg_count')),'comment_count':n(d.get('comment_count')),'forward_count':n(d.get('forward_count')),'repin_count':n(d.get('repin_count')),'image_count':n(d.get('image_count')),'content_schema_type':ctype,'has_video':d.get('has_video'),'schema':schema[:300]}
...
for kw in KEYWORDS:
    for endpoint in ['https://www.toutiao.com/search/','https://so.toutiao.com/search']:
        try:
            r=s.get(endpoint,params={'keyword':kw,'pd':'weitoutiao','source':'search_subtab_switch','dvpf':'pc'},timeout=30,allow_redirects=True)
            ...
            best={}
            for z in found:
                gid=z['group_id'];score=sum(bool(z.get(k)) for k in ['article_url','media_name','media_url','publish_time','digg_count','comment_count','repin_count'])
...
fields=['query','endpoint','group_id','title','abstract','article_url','media_name','media_url','user_id','publish_time','read_count','digg_count','comment_count','forward_count','repin_count','image_count','content_schema_type','has_video','schema']
with open('weitoutiao_probe.csv','w',newline='',encoding='utf-8-sig') as f:
    w=csv.DictWriter(f,fieldnames=fields);w.writeheader();w.writerows(out)
open('weitoutiao_report.json','w',encoding='utf-8').write(json.dumps(report,ensure_ascii=False,indent=2))
print(json.dumps({'queries':len(KEYWORDS),'requests':len(report),'unique_native':len(out),'max_read':max([z['read_count'] for z in out] or [0]),'max_digg':max([z['digg_count'] for z in out] or [0]),'max_comment':max([z['comment_count'] for z in out] or [0]),'max_forward':max([z['forward_count'] for z in out] or [0]),'max_repin':max([z['repin_count'] for z in out] or [0]),'digg_ge_100':sum(z['digg_count']>=100 for z in out),'digg_ge_1000':sum(z['digg_count']>=1000 for z in out),'comment_ge_100':sum(z['comment_count']>=100 for z in out),'comment_ge_1000':sum(z['comment_count']>=1000 for z in out),'repin_ge_100':sum(z['repin_count']>=100 for z in out),'repin_ge_1000':sum(z['repin_count']>=1000 for z in out)},ensure_ascii=False,indent=2))
