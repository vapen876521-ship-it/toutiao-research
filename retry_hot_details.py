...
GIDS=['7673362469359714826','7665671174047547919','7672981576782758400','7672693287387644456','7673685390176076303','7673054716309357065','7666317366066528806','7663012599079928339','7663676775276315170','7662201911889658418','7663805338268779051','7662946835924795942']
...
async def extract(page):
    return await page.evaluate("""() => {
      ...
      return {selector:best?best.s:'body',text:(best?best.t:(document.body.innerText||'')).slice(0,20000),paragraphs:ps.slice(0,160),images:[...new Set(imgs)].slice(0,100),links:links.slice(0,20)};
    }""")
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for gid in GIDS:
            ...
            rows.append(best);print(json.dumps({'gid':gid,'verified':best['detail_verified'],'status':best['http_status'],'chars':best['content_chars']},ensure_ascii=False));await asyncio.sleep(2.5)
        await b.close()
